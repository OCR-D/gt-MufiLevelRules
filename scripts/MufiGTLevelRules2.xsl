...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    xmlns:wt="https://github.com/dariok/w2tei" exclude-result-prefixes="#all" version="3.0">
            ...
            <xsl:when test="matches($hex, '([0-9]*|[A-F]*)')">
                <xsl:value-of select="
                        if ($hex = '') then
                            ...
                            $dec * math:pow(16, string-length($hex) - 1) + wt:hexToDec(substring($hex, 2))"
                />
            </xsl:when>
            <xsl:otherwise>
        ...
        <xsl:if test="$merge = 'yes'">
            <xsl:result-document format="xml_out" href="ghout/rules/megalevelrules.xml">
                <xsl:element name="levelrules">
                    <xsl:merge>
                        <xsl:merge-source
                            for-each-item="collection('../ghout/rules/characters/?select=*.xml')"
                            select="levelrules/ruleset">
    ...
    <xsl:template match="$MUFIEXPORT//fn:array" name="char">
        ...
        <xsl:for-each-group select="$MUFIEXPORT//fn:map" group-by="fn:string[@key = 'range']">
            ...
            <xsl:result-document format="txt_out"
                href="ghout/rules/characters/{fn:current-grouping-key()}.json"> {"ruleset":[
                    <xsl:variable name="keys"><line>
                        <xsl:for-each-group select="fn:current-group()"
                            group-by="fn:string[@key = 'alpha']">
                            <xsl:sort order="ascending" select="fn:string[@key = 'alpha']"/>
                            <xsl:for-each select="fn:current-group()">
                                <xsl:variable name="mufi">
                                    <xsl:choose>
                                        <xsl:when test="fn:string[@key = 'mufichar'] = '&quot;'"
                                            ><![CDATA[\"]]></xsl:when>
                                        <xsl:when test="fn:string[@key = 'mufichar'] = '\'"
                                            ><![CDATA[\\]]></xsl:when>
                                        <xsl:when test="contains(fn:string[@key = 'mufichar'], '◌')"
                                                ><xsl:value-of
                                                select="replace(fn:string[@key = 'mufichar'], '◌', '')"
                                            /></xsl:when>
                                        <xsl:otherwise><xsl:value-of
                                                select="fn:string[@key = 'mufichar']"
                                            /></xsl:otherwise>
                                    </xsl:choose>
                                </xsl:variable>
                                <xsl:variable name="c1">
                                    <xsl:for-each select="$OCRDrulesIMPORT//fn:array">
                                        <xsl:choose><xsl:when test="fn:string[1] = $mufi"
                                                  ><xsl:value-of select="$mufi"
                                                  /></xsl:when><xsl:otherwise><xsl:choose><xsl:when
                                                  test="fn:string[2] = $mufi"><xsl:value-of
                                                  select="fn:string[1]"
                                                  /></xsl:when><xsl:otherwise><xsl:choose><xsl:when
                                                  test="fn:string[3] = $mufi"><xsl:value-of
                                                  select="fn:string[1]"
                                                  /></xsl:when></xsl:choose></xsl:otherwise></xsl:choose></xsl:otherwise></xsl:choose>
                                    </xsl:for-each>
                                </xsl:variable>
                                <xsl:variable name="c2">
                                    <xsl:for-each select="$OCRDrulesIMPORT//fn:array"><xsl:if
                                            test="fn:string[3] = $mufi"><xsl:choose><xsl:when
                                                  test="fn:string[2] = $mufi">"<xsl:value-of
                                                  select="$mufi"
                                ...
                                <xsl:variable name="mufic2">
                                    <xsl:choose>
                                        <xsl:when test="contains(fn:string[@key = 'range'], 'PUA')">
                                            <xsl:choose>
                                                <xsl:when
                                                  test="contains(fn:string[@key = 'codepointalt'], '+')"
                                                  >"<xsl:for-each
                                                  select="fn:tokenize(fn:string[@key = 'codepointalt'], ' \+ ')">
                                                  <xsl:value-of
                                                  select="codepoints-to-string(wt:hexToDec(.))"
                                                  /></xsl:for-each>",</xsl:when>
                                                <xsl:otherwise/>
                                            </xsl:choose></xsl:when>
                                        <xsl:otherwise>"<xsl:value-of select="$mufi"
                                            />",</xsl:otherwise>
                                    </xsl:choose></xsl:variable>
                                <xsl:variable name="c3">
                                    <xsl:for-each select="$OCRDrulesIMPORT//fn:array">
                                        <xsl:choose><xsl:when test="fn:string[3] = $mufi"
                                                  ><xsl:value-of select="$mufi"
                                                  /></xsl:when><xsl:otherwise><xsl:choose><xsl:when
                                                  test="fn:string[2] = $mufi"><xsl:value-of
                                                  select="fn:string[3]"
                                ...
                                </xsl:variable> {"rule": [<xsl:choose>
                                    <!-- level1 -->
                                    <xsl:when test="fn:string[@key = 'range'] = 'BasLat'"
                                            >"<xsl:value-of select="$mufi"/>",</xsl:when><xsl:otherwise><xsl:choose>
                                            <xsl:when test="$c1 != ''">"<xsl:value-of select="$c1"
                                ...
                                <xsl:choose>
                                    <xsl:when test="$c2 = $mufic2"><xsl:choose>
                                            <xsl:when test="$mufic2 != ''"><xsl:value-of
                                                  select="$mufic2"
                                                  /></xsl:when><xsl:otherwise>"<xsl:value-of
                                                  select="$mufi"/>",</xsl:otherwise>
                                        </xsl:choose></xsl:when><xsl:otherwise><xsl:choose>
                                            <xsl:when test="$c2 != ''"><xsl:value-of select="$c2"
                                                /></xsl:when><xsl:otherwise><xsl:value-of
                                                  select="$mufic2"/></xsl:otherwise>
                                        </xsl:choose></xsl:otherwise>
                                </xsl:choose><!-- level3 -->
                                <xsl:choose>
                                    <xsl:when test="fn:string[@key = 'range'] = 'BasLat'"
                                            >"<xsl:value-of select="$mufi"/>"], "type":
                                        "level"}<komma/></xsl:when><xsl:otherwise><xsl:choose>
                                            <xsl:when test="$c3 != ''">"<xsl:value-of select="$c3"
                                                />"], "type":
                                                  "level"}<komma/></xsl:when><xsl:otherwise>"<xsl:value-of
                                                  select="$mufi"/>"], "type":
                                                "level"}<komma/></xsl:otherwise>
            ...
            <xsl:result-document format="xml_out"
                href="ghout/rules/characters/{fn:current-grouping-key()}.xml">
                        ...
                        <xsl:for-each select="fn:current-group()">
                            <xsl:variable name="mufi">
                                <xsl:choose>
                                    ...
                                    <xsl:otherwise>
                                        <xsl:value-of select="fn:string[@key = 'mufichar']"/>
                                    </xsl:otherwise>
                                </xsl:choose>
                            </xsl:variable>
                            ...
                            <xsl:variable name="c1">
                                <xsl:for-each select="$OCRDrulesIMPORT//fn:array">
                                    <xsl:choose>
                                        <xsl:when test="fn:string[1] = $mufi">
                                            <xsl:value-of select="$mufi"/>
                                        </xsl:when>
                                        <xsl:otherwise>
                                            <xsl:choose>
                                                <xsl:when test="fn:string[2] = $mufi">
                                                  <xsl:value-of select="fn:string[1]"/>
                                                </xsl:when>
                                                <xsl:otherwise>
                                                  <xsl:choose>
                                                  <xsl:when test="fn:string[3] = $mufi">
                                                  <xsl:value-of select="fn:string[1]"/>
                                                  </xsl:when>
                                                  </xsl:choose>
                                                </xsl:otherwise>
                            ...
                            <xsl:variable name="c2">
                                <xsl:for-each select="$OCRDrulesIMPORT//fn:array">
                                    <xsl:if test="fn:string[3] = $mufi">
                                        <xsl:choose>
                                            <xsl:when test="fn:string[2] = $mufi">
                                                <xsl:value-of select="$mufi"/>
                                            </xsl:when>
                                            <xsl:otherwise>
                                                <xsl:value-of select="fn:string[2]"/>
                                            </xsl:otherwise>
                                        </xsl:choose>
                                    </xsl:if>
                                </xsl:for-each>
                                ...
                                <xsl:for-each select="$OCRDrulesIMPORT//fn:array">
                                    <xsl:choose>
                                        ...
                                        <xsl:otherwise>
                                            <xsl:choose>
                                                <xsl:when test="fn:string[2] = $mufi">
                                                  <xsl:value-of select="fn:string[3]"/>
                                                </xsl:when>
                                            </xsl:choose>
                                        </xsl:otherwise>
                                    </xsl:choose>
                            ...
                            <xsl:choose>
                                <xsl:when test="fn:string[@key = 'range'] = 'BasLat'">
                                    <olevel/>
                                    <orange/>
                                    <xsl:value-of select="fn:string[@key = 'range']"/>
                                    <crange/>
                                    <odesc/>
                            ...
                            <xsl:choose>
                                <xsl:when test="fn:string[@key = 'range'] = 'BasLat'"
                                        ><orule/><xsl:value-of select="$mufi"
                                    /><crule/><otype/>level<ctype/><clevel/></xsl:when>
                                <xsl:otherwise>
                                    <xsl:choose>
                                        <xsl:when test="$c3 != ''"><orule/><xsl:value-of
                                                select="$c3"
                                            /><crule/><otype/>level<ctype/><clevel/></xsl:when>
                                        <xsl:otherwise><orule/><xsl:value-of select="$mufi"
                                            /><crule/><otype/>level<ctype/><clevel/></xsl:otherwise>
                                    </xsl:choose>
                                </xsl:otherwise>
                            </xsl:choose>
                        </xsl:for-each>
    ...
    <xsl:template match="olevel">
        <xsl:text disable-output-escaping="yes">&lt;ruleset&gt;</xsl:text>
    </xsl:template>
    <xsl:template match="clevel">
        <xsl:text disable-output-escaping="yes">&lt;/ruleset&gt;</xsl:text>
    </xsl:template>
    <xsl:template match="orule">
        <xsl:text disable-output-escaping="yes">&lt;rule&gt;</xsl:text>
    </xsl:template>
    <xsl:template match="crule">
        <xsl:text disable-output-escaping="yes">&lt;/rule&gt;</xsl:text>
    </xsl:template>
    <xsl:template match="otype">
        <xsl:text disable-output-escaping="yes">&lt;type&gt;</xsl:text>
    </xsl:template>
    <xsl:template match="ctype">
        <xsl:text disable-output-escaping="yes">&lt;/type&gt;</xsl:text>
    </xsl:template>
    <xsl:template match="olevelrules">
        <xsl:text disable-output-escaping="yes">&lt;levelrules&gt;</xsl:text>
    </xsl:template>
    <xsl:template match="clevelrules">
        <xsl:text disable-output-escaping="yes">&lt;/levelrules&gt;</xsl:text>
    </xsl:template>
    <xsl:template match="orange">
        <xsl:text disable-output-escaping="yes">&lt;range&gt;</xsl:text>
    </xsl:template>
    <xsl:template match="crange">
        <xsl:text disable-output-escaping="yes">&lt;/range&gt;</xsl:text>
    </xsl:template>
    <xsl:template match="odesc">
        <xsl:text disable-output-escaping="yes">&lt;desc&gt;</xsl:text>
    </xsl:template>
